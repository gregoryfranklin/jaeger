...
type ES7StorageIntegration struct {
	StorageIntegration
...
func (s *ES7StorageIntegration) initializeES(allTagsAsFields, archive bool) error {
	...
	client := eswrapper.WrapESClient7(s.client, s.bulkProcessor)
...
func (s *ES7StorageIntegration) esCleanUp(allTagsAsFields, archive bool) error {
	_, err := s.client.DeleteIndex("*").Do(context.Background())
	s.initSpanstore(allTagsAsFields, archive)
	return err
}

func (s *ES7StorageIntegration) initSpanstore(allTagsAsFields, archive bool) {
	bp, _ := s.client.BulkProcessor().BulkActions(1).FlushInterval(time.Nanosecond).Do(context.Background())
	client := eswrapper.WrapESClient7(s.client, bp)
	spanMapping, serviceMapping := es.GetMappings(5, 1, 7)
...
func (s *ES7StorageIntegration) esRefresh() error {
