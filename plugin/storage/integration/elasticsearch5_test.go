...
type ES5StorageIntegration struct {
	StorageIntegration
...
func (s *ES5StorageIntegration) initializeES(allTagsAsFields, archive bool) error {
	rawClient, err := elastic5.NewClient(
		elastic5.SetURL(queryURL),
		elastic5.SetBasicAuth(username, password),
	...
	client := eswrapper.WrapESClient5(s.client, s.bulkProcessor)
...
func (s *ES5StorageIntegration) esCleanUp(allTagsAsFields, archive bool) error {
	_, err := s.client.DeleteIndex("*").Do(context.Background())
	s.initSpanstore(allTagsAsFields, archive)
	return err
}

func (s *ES5StorageIntegration) initSpanstore(allTagsAsFields, archive bool) {
	bp, _ := s.client.BulkProcessor().BulkActions(1).FlushInterval(time.Nanosecond).Do(context.Background())
	client := eswrapper.WrapESClient5(s.client, bp)
	spanMapping, serviceMapping := es.GetMappings(5, 1, 5)
...
func (s *ES5StorageIntegration) esRefresh() error {
